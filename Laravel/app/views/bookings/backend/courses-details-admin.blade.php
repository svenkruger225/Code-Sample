				...
				<div>
					<select class="courseDate span7" id="courseDate{{$course->id}}" data-bind="'event': { 'change': updateSelectedInstance }" >
						...
						@foreach ( $course->instances as $instance )
							<option class="course-date-option @if ( $instance->isCourseCompleted() ) course-completed @elseif ( $instance->isCourseStarted() ) course-started @elseif ( $instance->full ) course-full @endif" value="{{$instance->id}}" >{{$instance->courseDateDescription}} <b>[ {{$instance->rosters->count()}}({{$instance->paid_total}}) {{$instance->maximum_students}} ]</b></option>
						@endforeach
					</select>                                          
