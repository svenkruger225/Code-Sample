...
<div id="content">
	...
	<div class="page-header">
		<div class="row-fluid">
			<div class="span12">  
				{{ Form::open(array('method' => 'POST', 'route' => 'backend.reports.exportmyob', 'class'=>'form-inline')) }}
					{{ Form::token() }}
	...
	<table class="table table-striped table-bordered table-hover table-condensed span10">
		...
		@foreach ($reports as $item)
			<tr>
        		...
        		<td>{{$item['size']}}</td>
        		<td>{{$item['date']}}</td>
				<td><a href="/backend/reports/downloadmyob?path={{$item['path']}}" target="_blank" class="btn btn-small btn-primary"> Download</a>
				<a href="/backend/reports/removemyob?path={{$item['path']}}" class="btn btn-small btn-danger"> Remove</a></td>
			</tr>
		@endforeach
			<tr><td colspan="4"> </td></tr>
			<tr><td colspan="4">total = {{count($reports)}} </td></tr>
